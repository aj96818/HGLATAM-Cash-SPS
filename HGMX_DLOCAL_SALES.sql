...
ADD	zVIND_GT VARCHAR(50)
	...
	,Overpayment_Flag VARCHAR(5)
	,Prepaid_Deposit_Flag VARCHAR(5)
--	,Sales_Amount_Local_Calc DECIMAL(15,2)
--	,Sales_Amount_USD_CALC DECIMAL(15,2)
...
WHERE ROW_TYPE IN ('TX')
	...
	AND	CAST(PROCESSING_DATE AS DATE) = CAST(@vDate_Datetime AS DATE)
...
UPDATE HGMX_DLOCAL SET FEES_CALC = (CASE WHEN TYPE = 'CREDIT' THEN PROCESSING_FEE_AMOUNT ELSE PROCESSING_FEE_AMOUNT * -1 END) 
UPDATE HGMX_DLOCAL SET COMPANY_CALC = 'HGMX'

-- Setting values for remaining HGMX_GT Table calculated fields:

UPDATE HGMX_GT SET Overpayment_Flag = (CASE WHEN Total_Transaction - Total_Invoice - VAT_Tax > 0 THEN 'X' ELSE '' END)
UPDATE HGMX_GT SET Prepaid_Deposit_Flag = (CASE WHEN SETTLEDBY_CALC = 'CREDIT' THEN 'X' ELSE '' END)
...
ON CAST(A.invoice_id AS VARCHAR) = CAST(B.TRANSACTION_ID AS VARCHAR)
...
UPDATE HGMX_GT_Summ_Tax SET ACCOUNT = (
		CASE 
			WHEN COMPANY_CALC = 'HGMX' AND TAX_TYPE_CALC = 'VAT' THEN '042-24910-000'
			WHEN COMPANY_CALC = 'HGMX' AND (TAX_TYPE_CALC = 'IST' OR TAX_TYPE_CALC = 'GST')	THEN '042-24940-000'
			WHEN COMPANY_CALC = 'HGMX' AND TAX_TYPE_CALC = 'UST' THEN '042-21120-000'
			ELSE 'XXX-XXXXX-XXX'
		END)
...
FROM (SELECT
		...
		, (CASE WHEN TYPE_CALC = 'CHARGEBACK' THEN 'CHARGEBACK' ELSE 'N/A' END) CHARGEBACK
		, DATE AS 'GT_DATE'
	FROM HGMX_GT
	WHERE SETTLEDBY_CALC = 'dlocal'
	GROUP BY Invoice_id, SettledBy_CALC, Currency, COMPANY_CALC, DATE, zvind_GT, (CASE WHEN TYPE_CALC = 'CHARGEBACK' THEN 'CHARGEBACK' ELSE 'N/A' END)) GT
FULL OUTER JOIN 
	(SELECT
		ISNULL(TRANSACTION_ID, '') as TRANSACTION_ID
		, COMPANY_CALC
		-- ISNULL(Payment_type, 'Unknown') Payment_Type, TYPE
		, PROCESSING_CURRENCY
		, SUM(CASE WHEN TYPE = 'CREDIT' THEN (ISNULL(PROCESSING_AMOUNT, 0) * -1) ELSE ISNULL(PROCESSING_AMOUNT, 0) END) PROCESSING_AMOUNT
		--, FX_RATE
		, SUM(CASE WHEN TYPE = 'CREDIT' THEN (ISNULL(NET_SETTLEMENT_AMOUNT, 0) * -1) ELSE ISNULL(NET_SETTLEMENT_AMOUNT, 0) END) NET_SETTLEMENT_AMOUNT
		, SUM(CASE WHEN TYPE = 'CREDIT' THEN (ISNULL(GROSS_SETTLEMENT_AMOUNT, 0) * -1) ELSE ISNULL(GROSS_SETTLEMENT_AMOUNT, 0) END) GROSS_SETTLEMENT_AMOUNT
		, CAST(PROCESSING_DATE AS DATE) DL_PROCESSING_DATE
	FROM HGMX_DLOCAL
	GROUP BY
		TRANSACTION_ID, COMPANY_CALC, Processing_Currency, CAST(PROCESSING_DATE AS DATE)
	) DLOCAL
		ON DLOCAL.TRANSACTION_ID = GT.INVOICE_ID
		AND GT.CHARGEBACK = 'N/A'
...
ADD ZDIFF_USD DECIMAL(15,2),
	...
	, Hidden_Fee_Rate DECIMAL(10,2)
	, Hidden_Fee_Calc DECIMAL(15,2)

UPDATE HGMX_GT_DLOCAL_FULLOUTERJOIN
  SET Hidden_Fee_Rate = (CASE WHEN Currency = 'COP' THEN 0.1 
								WHEN Currency = 'CLP' THEN 0.09
								WHEN Currency = 'MXN' THEN 0.07 
								ELSE NULL END)

UPDATE HGMX_GT_DLOCAL_FULLOUTERJOIN
  SET Hidden_Fee_Calc = 
		  	(CASE 
				WHEN TRANS_AMOUNT_LOCAL_CALC = PROCESSING_AMOUNT THEN (TRANS_AMOUNT_USD_CALC * Hidden_Fee_Rate)
				ELSE 0
			 END)
...
UPDATE HGMX_GT_DLOCAL_FULLOUTERJOIN SET ZFX_DIFF = 
			(CASE 
				WHEN TRANS_AMOUNT_LOCAL_CALC = PROCESSING_AMOUNT THEN ((TRANS_AMOUNT_USD_CALC * (1 - Hidden_Fee_Rate)) - GROSS_SETTLEMENT_AMOUNT)
				ELSE 0
			 END)
...
UPDATE HGMX_DLOCAL_FX_ADJ SET ACCOUNT = (CASE WHEN COMPANY_CALC = 'HGMX' THEN '061-82700-950' ELSE 'XXX-XXXXX-XXX' END)
...
UPDATE HGMX_DLOCAL_FX_ADJ SET DISTREF = COMPANY_CALC + ' (REALIZED (GAIN) LOSS) ' + PROCESSING_CURRENCY
...
-- Create DLOCAL Hidden Fees Journal Entries (incorporated 1/19/21)
SELECT
	COMPANY_CALC
	, PROCESSING_CURRENCY
	, SUM(Hidden_Fee_Calc) Hidden_Fees_Calc
INTO
	HGMX_DLOCAL_Hidden_Fees
FROM
	HGMX_GT_DLOCAL_FULLOUTERJOIN
WHERE ABS(Hidden_Fee_Calc) <> 0
GROUP BY
	COMPANY_CALC
	, PROCESSING_CURRENCY

ALTER TABLE HGMX_DLOCAL_Hidden_Fees ADD CHECKBOOKID VARCHAR(50), BATCHID VARCHAR(50), TRANTYPE VARCHAR(50), TRANDATE VARCHAR(50), SRCDOC VARCHAR(50), CURRID VARCHAR(50), REFERENCE VARCHAR(50), ACCOUNT VARCHAR(50), DEBIT DECIMAL(15, 2), CREDIT DECIMAL(15, 2), DISTREF VARCHAR(50), KEY1 VARCHAR(50), REVERSEDATE VARCHAR(50), UNIQUEID VARCHAR(50), table_name VARCHAR(55);

UPDATE HGMX_DLOCAL_Hidden_Fees SET table_name = 'HGMX_DLOCAL_Hidden_Fees'
UPDATE HGMX_DLOCAL_Hidden_Fees SET CHECKBOOKID = 'Operating'
UPDATE HGMX_DLOCAL_Hidden_Fees SET BATCHID = 'ACL_' + FORMAT(@vDate_Datetime, 'MMddyyyy')
UPDATE HGMX_DLOCAL_Hidden_Fees SET TRANTYPE = 5
UPDATE HGMX_DLOCAL_Hidden_Fees SET TRANDATE = FORMAT(@vDate_Datetime, 'MMddyyyy')
UPDATE HGMX_DLOCAL_Hidden_Fees SET SRCDOC = 'ACL'
UPDATE HGMX_DLOCAL_Hidden_Fees SET CURRID = 'Z-US$'
UPDATE HGMX_DLOCAL_Hidden_Fees SET REFERENCE = FORMAT(@vDate_Datetime, 'MMddyyyy') + ' REVENUE DLOCAL';
UPDATE HGMX_DLOCAL_Hidden_Fees SET ACCOUNT = (CASE WHEN COMPANY_CALC = 'HGMX' THEN '061-57000-590' ELSE 'XXX-XXXXX-XXX' END)
UPDATE HGMX_DLOCAL_Hidden_Fees SET DEBIT = (CASE WHEN Hidden_Fees_Calc < 0 THEN 0 ELSE Hidden_Fees_Calc END)
UPDATE HGMX_DLOCAL_Hidden_Fees SET CREDIT = (CASE WHEN Hidden_Fees_Calc > 0 THEN 0 ELSE Hidden_Fees_Calc * -1 END)
UPDATE HGMX_DLOCAL_Hidden_Fees SET DISTREF = COMPANY_CALC + ' Hidden Fees ' + PROCESSING_CURRENCY
UPDATE HGMX_DLOCAL_Hidden_Fees SET KEY1 = '';
UPDATE HGMX_DLOCAL_Hidden_Fees SET REVERSEDATE = ''
UPDATE HGMX_DLOCAL_Hidden_Fees SET UNIQUEID = 'HGMXDLF' + FORMAT(@vDate_Datetime,'MMddyyyy')

INSERT INTO HGMX_DLOCAL_JE (CHECKBOOKID, BATCHID, TRANTYPE, TRANDATE, SRCDOC, CURRID, REFERENCE, ACCOUNT, DEBIT, CREDIT, DISTREF, KEY1, REVERSEDATE, UNIQUEID, table_name)
SELECT CHECKBOOKID, BATCHID, TRANTYPE, TRANDATE, SRCDOC, CURRID, REFERENCE, ACCOUNT, DEBIT, CREDIT, DISTREF, KEY1, REVERSEDATE, UNIQUEID, table_name
FROM HGMX_DLOCAL_Hidden_Fees
--WHERE ABS(Hidden_Fees_Calc) > 0
...
SELECT 
	...
	, ISNULL(CHARGEBACK, 'N/A') CHARGEBACK
INTO HGMX_DLOCAL_Summ_Exceptions
FROM HGMX_DLOCAL_EXCEPTION_REPORT
GROUP BY ALL_COMPANY, ISNULL(CHARGEBACK, 'N/A')
...
UPDATE HGMX_DLOCAL_Summ_Exceptions SET ACCOUNT = (CASE WHEN CHARGEBACK = 'CHARGEBACK' THEN '061-11035-000' ELSE '061-11045-000' END) 		
...
UPDATE HGMX_DLOCAL_Summ_Exceptions SET DISTREF = (CASE WHEN CHARGEBACK = 'CHARGEBACK' THEN ALL_COMPANY + ' CHARGEBACK SUSPENSE (DLOCAL)' ELSE ALL_COMPANY + ' A/R SUSPENSE (DLOCAL)' END)
...
-- Not sure if Chargeback are being properly reported in the Dlocal report (?)
SELECT *
...
UPDATE HGMX_DLOCAL_CBACKS SET NET_AMT_USD_CALC = (
			CASE 
			WHEN TYPE = 'CREDIT'
			THEN - 1 * (isnull(dispute_settlement_amount, 0) + isnull(dispute_fee, 0))
			ELSE isnull(dispute_settlement_amount, 0) + isnull(dispute_fee, 0)
			END);
...
SELECT
	COMPANY_CALC
	,CAST(ISNULL(PROCESSING_DATE, '') as DATE) AS DL_PROCESSING_DATE
	,SUM((CASE WHEN TYPE = 'CREDIT' THEN ISNULL(NET_SETTLEMENT_AMOUNT, 0) * -1 ELSE ISNULL(NET_SETTLEMENT_AMOUNT, 0) END)) AS DL_NET_SETTLE_AMT
INTO DLOCAL_AR_Summ_Temp
FROM HGMX_DLOCAL
WHERE PROCESSING_DATE IS NOT NULL
GROUP BY
	COMPANY_CALC
	,CAST(ISNULL(PROCESSING_DATE, '') AS DATE);

SELECT
	COMPANY_CALC
	,CAST(CB_VALUE_DATE AS DATE) AS DL_PROCESSING_DATE
	,SUM(CASE WHEN TYPE = 'CREDIT'	THEN - 1 * (dispute_settlement_amount+dispute_fee) ELSE dispute_settlement_amount+dispute_fee END) AS NET_CB_AMT_CALC
INTO DLOCAL_AR_Summ_Cbacks
FROM HGMX_DLOCAL_CBACKS
--WHERE PROCESSING_DATE IS NOT NULL
GROUP BY COMPANY_CALC
	,CAST(CB_VALUE_DATE as date);

SELECT a.company_calc
	,CAST(a.DL_PROCESSING_DATE as date) as DL_PROCESSING_DATE
	,a.DL_NET_SETTLE_AMT 
...
INTO HGMX_AR_TOTAL
FROM HGMX_AR_JE
WHERE DISTREF = 'HGMX - ACCOUNTS RECEIVABLE (DLOCAL)'
GROUP BY CHECKBOOKID

ALTER TABLE HGMX_AR_TOTAL ADD DOCAMTDLS DECIMAL(15,2);

UPDATE HGMX_AR_TOTAL SET DOCAMTDLS = DEBIT - CREDIT;

ALTER TABLE HGMX_DLOCAL_JE
ADD DOCAMT DECIMAL(15,5)
, DISTYPE INT;

UPDATE HGMX_DLOCAL_JE SET DOCAMT = (SELECT DOCAMTDLS FROM HGMX_AR_TOTAL)
UPDATE HGMX_DLOCAL_JE SET DISTYPE = (CASE WHEN ACCOUNT <> '061-11001-000' THEN 9 ELSE 3 END)
...
SELECT sum(credit) - sum(debit) FROM HGMX_DLOCAL_SALES_JE_FINAL
