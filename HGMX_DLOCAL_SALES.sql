...
-- All Matched & Un-matched Txns between GT & DLOCAL:

SELECT *
INTO HGMX_GT_DLOCAL_FULLJOIN
FROM (SELECT
...
FULL JOIN 
	(SELECT
		...
		, PROCESSING_CURRENCY
		, (CASE WHEN TYPE = 'CREDIT' THEN (ISNULL(PROCESSING_AMOUNT, 0) * -1) ELSE ISNULL(PROCESSING_AMOUNT, 0) END) PROCESSING_AMOUNT
		, FX_RATE
		, (CASE WHEN TYPE = 'CREDIT' THEN (ISNULL(NET_SETTLEMENT_AMOUNT, 0) * -1) ELSE ISNULL(NET_SETTLEMENT_AMOUNT, 0) END) NET_SETTLEMENT_AMOUNT
		, (CASE WHEN TYPE = 'CREDIT' THEN (ISNULL(GROSS_SETTLEMENT_AMOUNT, 0) * -1) ELSE ISNULL(GROSS_SETTLEMENT_AMOUNT, 0) END) GROSS_SETTLEMENT_AMOUNT
		, PROCESSING_DATE
	FROM HGMX_DLOCAL) DLOCAL
ON DLOCAL.TRANSACTION_ID = GT.INVOICE_ID
AND PROCESSING_CURRENCY=CURRENCY

ALTER TABLE HGMX_GT_DLOCAL_FULLJOIN
ADD ZDIFF_USD DECIMAL(15,2),
	ZDIFF_ABS DECIMAL(15,2),
	ZOTHER_DIFF DECIMAL (15,2)
	, zFX_Diff DECIMAL(15,2)

UPDATE HGMX_GT_DLOCAL_FULLJOIN
...
UPDATE HGMX_GT_DLOCAL_FULLJOIN SET ZDIFF_USD = TRANS_AMOUNT_USD_CALC - GROSS_SETTLEMENT_AMOUNT
UPDATE HGMX_GT_DLOCAL_FULLJOIN SET ZDIFF_ABS = ABS(ZDIFF_USD)
UPDATE HGMX_GT_DLOCAL_FULLJOIN SET ZFX_DIFF = 
			(CASE 
				WHEN TRANS_AMOUNT_LOCAL_CALC = PROCESSING_AMOUNT THEN TRANS_AMOUNT_USD_CALC - GROSS_SETTLEMENT_AMOUNT
				ELSE 0
			 END)
UPDATE HGMX_GT_DLOCAL_FULLJOIN SET ZOTHER_DIFF = ZDIFF_USD - ZFX_DIFF


-- Start of DLOCAL Exception Report
--select * from HGMX_GT_DLOCAL_FULLOUTERJOIN where type = 'credit'
-- see if TRANS_AMOUNT_USD_CALC == REVENUE_Amount_USD_CALC (because no tax on 9/25/20)
--select sum(ISNULL(TRANS_AMOUNT_USD_CALC, 0)), SUM(ISNULL(REVENUE_AMOUNT_USD_CALC, 0)) FROM HGMX_GT_DLOCAL_FULLOUTERJOIN
--select sum(revenue_amount_usd_calc) from hgmx_gt where invoice_id = '391970'

--select * from hgmx_gt where Invoice_ID = '391970'
--select * from HGMX_GT_DLOCAL_FULLOUTERJOIN where invoice_id = '391970'
--select * from HGMX_DLOCAL where transaction_id = '391970'

--select sum(REVENUE_AMOUNT_USD_CALC) from hgmx_gt where SettledBy_CALC = 'dlocal'
--select count(*) from HGMX_GT_DLOCAL_FULLOUTERJOIN
