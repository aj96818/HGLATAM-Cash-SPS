USE [GTStage_Matt]
GO
/****** Object:  StoredProcedure [dbo].[HGMX_DLOCAL_SALES]    Script Date: 10/16/2020 9:44:39 AM ******/
SET ANSI_NULLS ON
GO
...
--select sum(zother_diff) from HGMX_GT_DLOCAL_FULLOUTERJOIN where type = 'credit'
SELECT *
...
FULL OUTER JOIN 
	(SELECT
		ISNULL(TRANSACTION_ID, '') as TRANSACTION_ID
		, COMPANY_CALC, Payment_type, TYPE
		, PROCESSING_CURRENCY
		, (CASE WHEN TYPE = 'CREDIT' THEN (ISNULL(PROCESSING_AMOUNT, 0) * -1) ELSE ISNULL(PROCESSING_AMOUNT, 0) END) PROCESSING_AMOUNT
		, FX_RATE
		, (CASE WHEN TYPE = 'CREDIT' THEN (ISNULL(NET_SETTLEMENT_AMOUNT, 0) * -1) ELSE ISNULL(NET_SETTLEMENT_AMOUNT, 0) END) NET_SETTLEMENT_AMOUNT
