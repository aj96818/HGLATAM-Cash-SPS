USE [GTStage_Matt]
GO
/****** Object:  StoredProcedure [dbo].[HGMX_EXCEPT_TEST_LOOP]    Script Date: 9/3/2020 9:02:35 AM ******/
SET ANSI_NULLS ON
GO
...
SELECT RIGHT(TRANDATE,4) + LEFT(TRANDATE, 4) DATE, SUM(DEBIT - CREDIT) SUSPENSE FROM HGMX_DLOCAL_SALES_JE_ALL WHERE ACCOUNT LIKE '%11045%' GROUP BY RIGHT(TRANDATE, 4) + LEFT(TRANDATE, 4);
SELECT RIGHT(TRANDATE,4) + LEFT(TRANDATE, 4) DATE, SUM(DEBIT - CREDIT) SUSPENSE FROM HGMX_PAYPAL_SALES_JE_ALL WHERE ACCOUNT LIKE '%11045%' GROUP BY RIGHT(TRANDATE, 4) + LEFT(TRANDATE, 4);
SELECT RIGHT(TRANDATE,4) + LEFT(TRANDATE, 4) DATE, SUM(DEBIT - CREDIT) SUSPENSE FROM HGMX_PAYU_SALES_JE_ALL WHERE ACCOUNT LIKE '%11045%' GROUP BY RIGHT(TRANDATE, 4) + LEFT(TRANDATE, 4);
