USE [GTStage_Matt]
GO
/****** Object:  StoredProcedure [dbo].[HGMX_OVERPAYMENT_SALES]    Script Date: 12/2/2020 10:24:23 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER     PROCEDURE [dbo].[HGMX_OVERPAYMENT_SALES] @vDate VARCHAR(15) AS 
...
UPDATE HGMX_GT_OVERPAYMENT SET Overpayment = ISNULL(Total_Transaction, 0) - ISNULL(Total_Invoice, 0) - ISNULL(VAT_Tax, 0)
UPDATE HGMX_GT_OVERPAYMENT SET Overpayment_USD = (ISNULL(Total_Transaction, 0) - ISNULL(Total_Invoice, 0) - ISNULL(VAT_Tax, 0)) * ISNULL(FX_RATE_INVRS, 0)
...
SELECT CHECKBOOKID, BATCHID, TRANTYPE, TRANDATE, SRCDOC, CURRID, REFERENCE, ACCOUNT, DEBIT, CREDIT, DISTREF, KEY1, REVERSEDATE, UNIQUEID, DOCAMT, DISTYPE, table_name
...
ON A.COMPANY_CALC = B.Company_Calc
--AND A.TYPE_CALC = B.Type_Calc
AND 'Cust Deposit' = B.Business_Line_Calc 
...
UPDATE HGMX_Overpayment_JE SET DOCAMT = (SELECT SUM(DEBIT) FROM HGMX_Overpayment_JE)
UPDATE HGMX_Overpayment_JE SET DISTYPE = '9'
